...
WITH Source_Distinct AS (
    -- Count the unique policies arriving from Staging
    SELECT COUNT(DISTINCT POL_NBR) AS Total_Inbound_Policies
    FROM dbo.STG_POLICY
)
SELECT 
    Total_Inbound_Policies,
    (SELECT COUNT(*) FROM dbo.ILM_POLICY) AS Target_Loaded,
    (SELECT COUNT(*) FROM dbo.ERROR_POLICY) AS Error_Rejected,
    
    -- The Variance MUST be 0. If it is not 0, a silent drop occurred.
    Total_Inbound_Policies - (
        (SELECT COUNT(*) FROM dbo.ILM_POLICY) + 
        (SELECT COUNT(*) FROM dbo.ERROR_POLICY)
    ) AS Record_Variance
